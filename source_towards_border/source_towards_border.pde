...
void setup() {
  ...
  trails.add(new PointWithTrail(0));
  trails.add(new PointWithTrail(50));
  trails.add(new PointWithTrail(100));
  trails.add(new PointWithTrail(150));
  trails.add(new PointWithTrail(200));
}
...
class PointWithTrail {
  ...
  PointWithTrail(float targetY) {
    ...
    for(int i=0; i<nBallsPerStep; i++) {
      ...
      float theta = atan((base.y - targetY) / (base.x - targetX));
      vs.add(randomVector(base, hue, 0));
    }    
  }
